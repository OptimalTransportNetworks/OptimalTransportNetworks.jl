# The primal case, with mobility and cross-good congestion, and an Armington (1969) world where each location produces only one good

function model_mobility_cgc_armington(optimizer, auxdata)
...
function recover_allocation_mobility_cgc_armington(model, auxdata)
    param = auxdata.param
    graph = auxdata.graph
