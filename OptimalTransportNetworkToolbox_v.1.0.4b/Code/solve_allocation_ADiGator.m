...
[results,flag,x]=solve_allocation_ADiGator(...): 
...
function [results,flag,x]=solve_allocation_ADiGator(x0,auxdata,funcs,verbose)
...
funcs.gradient = @(x) objective_Grd(x,auxdata);
funcs.constraints = @(x) constraints(x,auxdata);
funcs.jacobian = @(x) constraints_Jac(x,auxdata);
funcs.hessian = @(x,sigma,lambda) objective_Hes(x,auxdata,sigma,lambda);
