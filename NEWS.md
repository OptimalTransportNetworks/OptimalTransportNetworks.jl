...
* **`optimal_network()` did not pass the network bounds `Il`/`Iu` on to `annealing()`,** so simulated annealing silently ran unconstrained (`Il = zeros(J, J)`, `Iu = Inf`). Both are in scope at the call site and are simply now forwarded. On top of ignoring what the caller asked for, this combined badly with the `delta_i == 0` guards restored in 0.3.2: the deepening step sets `I1[graph.delta_i .== 0] .= 0`, and with `Il = 0` there is no lower bound for `rescale_network!()` to clamp those edges back up to, so they stay at zero infrastructure — `kappa = 0`, infinite transport cost — and the next allocation solve becomes badly ill-conditioned. On the CEMAC road network (10 edges with `delta_i = 0`) the first annealing draw did not converge: Ipopt ran 652 iterations with a trust-region radius of ~4e5 and made no progress. Controlled check on that network, same starting network and same RNG seed, varying only the bounds: forwarded -> 3 draws in 75 s; dropped -> still stuck after 420 s.

* **`Il` and `Iu` could not actually be passed to `annealing()`.** `retrieve_options_annealing()` normalised every keyword with `Symbol(lowercase(string(k)))`, so the documented `Il`/`Iu` arguments became `:il`/`:iu`, which are not keys of the options dict — raising `Unknown parameter: il` rather than setting the bound. Keys are now matched case-insensitively and written back to the canonical key, so `annealing(param, graph, I0; Il = ..., Iu = ...)` works as documented.
